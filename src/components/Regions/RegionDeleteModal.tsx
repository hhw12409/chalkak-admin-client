...
/**
 * 지역 마스터 물리 삭제 확인 모달.
 * - 도장깨기 집계 영향 경고 + 이름 일치 입력 검증.
 * - 비활성(active 토글) 권장 안내.
 */
export default function RegionDeleteModal({ region, onClose, onSuccess }: Props) {
  const [confirmText, setConfirmText] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");

  const canDelete = confirmText === region.name && !submitting;
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await regionsApi.remove(region.regionId);
      ...
      <div className="w-full max-w-md rounded-sm bg-white p-6 shadow-lg dark:bg-boxdark">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            placeholder={`삭제하려면 "${region.name}"을 입력하세요`}
