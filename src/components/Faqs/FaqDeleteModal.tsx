...
export default function FaqDeleteModal({ faq, onClose, onSuccess }: Props) {
  const [confirmText, setConfirmText] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");

  const expected = `FAQ-${faq.faqId}`;
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await faqsApi.deleteFaq(faq.faqId);
