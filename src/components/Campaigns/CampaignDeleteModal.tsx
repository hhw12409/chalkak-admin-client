...
/**
 * 캠페인 soft delete 확인 모달.
 * - 제목 일치 입력 검증(실수 방지). 서버는 status=DELETED 처리(복구 불가).
 */
export default function CampaignDeleteModal({ campaign, onClose, onSuccess }: Props) {
  const [confirmText, setConfirmText] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");

  const canDelete = confirmText === campaign.title && !submitting;
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await campaignsApi.remove(campaign.campaignId);
      ...
      <div className="w-full max-w-md rounded-sm bg-white p-6 shadow-lg dark:bg-boxdark">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            placeholder={`삭제하려면 "${campaign.title}"을 입력하세요`}
